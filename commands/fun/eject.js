...
module.exports = {
    config: {
        name: 'eject',
        description: "Vous éjecte dans l'espace (Among Us)",
        aliases: ["eject"],
        usage: '<user>',
        category: 'fun',
    },
...
const user = message.mentions.users.first() || (args.length > 0 ? message.users.cache.filter(e => e.username.toLowerCase().includes(args.join(" ").toLowerCase())).first(): message.author) || message.author;
    ...
    const embed = new Discord.MessageEmbed()
      ...
      .setTitle(`${message.author.username} a décidé d'éjecter ${user.username}`)
    ...
    .setTitle(`${bot.emotes.error} Quelque chose a mal tourné.\n${bot.emotes.error}Note : Cela ne fonctionnera pas si l'utilisateur contient des caractères non désirés dans son nom d'utilisateur..`)
    .setColor(config.embedcolor);
