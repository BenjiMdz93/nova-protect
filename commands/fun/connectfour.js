...
module.exports = {
    config: {
        ...
        aliases: ['connect4', 'c4'],
        category: 'fun',
        usage: '[name | nickname | mention',
        description: "Jouez à un jeu de Puissance 4 avec un autre utilisateur jusqu'à ce que quatre lignes horizontales ou verticales soient reliées.",
    },
    run: async (bot, message, args, ops) => {
        ...
        if (opponent.user.bot) return message.channel.send('**Il est interdit de jouer contre des robots!**');
        ...
        try {
            ...
            while (!winner && board.some(row => row.includes(null))) {
                ...
                await message.channel.send(stripIndents`
					**${user}, Quelle colonne voulez-vous choisir ? Tapez ``end`` pour déclarer forfait.!**
					${displayBoard(board)}
					${nums.join('')}
				`);
                const filter = res => {
            ...
            return message.channel.send(winner ? `**Congrats, ${winner}!**` : "**C'est un match nul!**");
        } catch (err) {
            ops.games.delete(message.channel.id);
            throw err;
        }
